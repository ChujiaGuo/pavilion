...
**Auth guard — `useRequireAuth` (`src/client/src/lib/hooks/use-require-auth.ts`).** Client-side hook shared by `/home` and every other page behind `AppShell`: on mount, checks `supabase.auth.getSession()` and redirects to `/login` if there's no session, or `/onboarding/quiz` if the session's `app_metadata.onboarding_completed` isn't set — same two-part check `onboarding/quiz/page.tsx` already did inline, generalized so every gated page doesn't reimplement it. Callers render `null` until the hook returns a non-null `{ accessToken, userId }`, matching the quiz page's existing pattern. This is a client-side redirect, not middleware — there's no Next.js `middleware.ts` in this app (see "Auth" above: session state lives in `@supabase/ssr`'s browser client, not a cookie a server-side middleware would refresh), so an unauthenticated direct nav to `/home` briefly resolves the route before the guard fires, rather than being blocked at the edge. Acceptable since no sensitive data renders before the guard redirects — the page returns `null` until `useRequireAuth` resolves.
