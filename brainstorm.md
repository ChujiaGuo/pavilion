...
| Branded confirm-signup email | Fully implemented and ready — `supabase/templates/confirm_signup.html`, `/auth/confirm` route, `/email-confirmed` page (see technical-notes.md "Auth") — but inactive: `supabase config push` rejected `[auth.email.template.confirmation]` because this project is on the free tier using Supabase's default mail relay, and template customization is plan-gated without a custom SMTP provider configured. Signups today go through GoTrue's plain default confirmation email instead. Needs either custom SMTP (SendGrid/Postmark/etc. via `[auth.email.smtp]`) or a plan upgrade before the config can be un-commented and pushed — no further code work required, just an infra/billing decision. | v2 |
