...
**`/profile` — implemented, view + edit + logout.** `src/client/src/app/profile/page.tsx`, reached via `AppNav`'s fourth destination. View mode composes the caller's own data from two existing endpoints — `GET /api/users/:id` (name, city/region, preferred formats, play style, privacy level, member-since) and `GET /api/ratings/user/:userId` (grade/subtier label, provisional flag) — no new endpoint, and no `getUserProfile()`-style aggregation was added (see "User profile fields not yet exposed" below: `reliabilityScore`/`sessionCount` still aren't exposed by any endpoint, so they don't appear here either). When `rating.isProvisional` is true, the "Provisional" badge carries an `InfoTooltip` (design-system.md's Component Library) explaining what that means in plain language — the API already returns `isProvisional` as a boolean with no explanatory text, so the copy lives entirely client-side.
