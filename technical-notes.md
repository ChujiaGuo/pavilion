...
**Google OAuth — code path implemented, provider disabled pending real credentials.** `GoogleSignInButton` (`src/client/src/components/auth/google-sign-in-button.tsx`, used on both signup and login) calls `signInWithOAuth({ provider: 'google', options: { redirectTo: '.../auth/callback' } })` on the browser client. `src/client/src/app/auth/callback/route.ts` is the PKCE callback: it exchanges the `code` param for a session via a cookie-aware server client (`src/client/src/lib/supabase/server.ts`, `createServerClient` from `@supabase/ssr` reading/writing cookies through `next/headers`) and redirects to `/`, or to `/login?error=oauth_failed` on failure — no server-side change needed beyond this, since `src/server/src/middleware/auth.ts` validates any provider's token the same way. `supabase/config.toml`'s `[auth.external.google]` block is scaffolded but `enabled = false`: it needs a real Google Cloud OAuth client ID/secret (set via the `SUPABASE_AUTH_EXTERNAL_GOOGLE_CLIENT_ID`/`_SECRET` env vars it references) before it does anything locally — see README.md "Enabling Google sign-in (optional)" for the steps to actually turn it on. The same client_id/secret need to be set in the Supabase dashboard for prod (config.toml only governs local dev).
