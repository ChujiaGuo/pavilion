...
**"Recent activity"'s `status=completed` fetch is currently dormant in real usage.** Since the Session Status Lifecycle reorder (technical-notes.md), `completed` is unreachable through the live app — `active` now advances to `voting`, which has no exit yet — so this section only ever shows something for seeded/admin-edited data today, and otherwise degrades to its honest "No sessions played yet." empty state rather than showing nothing at all silently. Left as-is rather than redefined (e.g. to also cover `voting`, which would just duplicate "Ready to rate" below) since there are several reasonable ways to redefine "recent" once `voting → completed` actually exists, and picking one now would be premature.

**"Ready to rate" follows the same reuse pattern for `voting`-status sessions.** Fetches `attendee_id=<self>&status=voting` and `organizer_id=<self>&status=voting` in parallel, merges, de-dupes by session id (unlike "Next session," this section renders every match as a list row rather than picking just one, so a session where the caller is both organizer and an RSVP'd attendee would otherwise show twice), and sorts newest-first. Shows up as soon as a session enters `voting` — including before its organizer has marked attendance, since that's now the first step *within* voting rather than a precondition for reaching it. Unlike the other two widgets, this section renders nothing at all when the list is empty — no persistent empty-state box — since it's a transient, actionable item rather than a permanent dashboard fixture, and it keeps `home-dashboard.spec.ts`'s empty-state assertions from needing a new exclusion.
...
**Voting-stage client UI is not implemented.** A session reaches `voting` via the organizer/moderator's `active → voting` action, and `markAttendance` — the first step within voting, not a gate into it — requires that status and never changes it further (technical-notes.md's "Session Status Lifecycle"). `POST /api/ratings/submit` already works against a `voting` session in principle, but its eligibility check currently requires `session_rsvps.status = 'going'`, which `markAttendance` clears the moment it runs — see technical-notes.md's KI-008. Regardless, nothing in the client actually lets a player cast those post-session ratings yet: the session detail page and `/home` dashboard only surface that a session has *entered* the voting stage (a status note and a "Ready to rate" list respectively); building the actual vote-casting page/component is future work.
