...
| Branded confirm-signup email | The *email itself* — `supabase/templates/confirm_signup.html`, styled to match the app — is built but inactive: `supabase config push` rejected `[auth.email.template.confirmation]` because this project is on the free tier using Supabase's default mail relay, and template *content* customization is plan-gated without a custom SMTP provider configured. Signups today get GoTrue's plain default confirmation email. The surrounding UX isn't degraded, though — `emailRedirectTo` (unaffected by the plan restriction) still routes the confirmed user through `/email-confirmed` into an already-logged-in `/home`, and `/auth/confirm` (the branded template's intended link target) is fully implemented and tested, just unused while the template's inactive. Needs either custom SMTP (SendGrid/Postmark/etc. via `[auth.email.smtp]`) or a plan upgrade before the template config can be un-commented and pushed — no further code work required, just an infra/billing decision. | v2 |
