...
**CORS — API-side, `src/server/src/index.ts`.** Replaced the earlier open `cors()` default with an explicit allow-list read from `ALLOWED_ORIGINS` (comma-separated, no wildcard support — see `.env.example`), defaulting to `http://localhost:3000` in dev. Also restricts `allowMethods` to `['GET', 'POST', 'PATCH', 'DELETE']` and `allowHeaders` to `['Content-Type', 'Authorization']`. Prod sets this to the actual frontend domain(s) — see README.md "Production setup (one-time)" for the env var walkthrough.
...
| `PATCH` | `/:id` | yes | Update own profile fields (`displayName`, `firstName`, `lastName`, `photoUrl`, `city`, `region`, `preferredFormats`, `playStyle`, `privacyLevel`). Returns 403 if patching a different user. `firstName`/`lastName` are optional unless the user already holds a `verifiedTier` — see "Verification approval action" below |
...
- **Avoiding a login-time fetch:** rather than a network round-trip on every login to learn onboarding status, `completeOnboarding` also best-effort syncs `app_metadata.onboarding_completed = true` via `supabase.auth.admin.updateUserById` (the admin-only `app_metadata`, not the client-editable `user_metadata`). `signInWithPassword`'s response always carries a freshly-minted token reflecting current DB state, so `login/page.tsx` reads `data.user.app_metadata.onboarding_completed` synchronously off that response, and the quiz page's own guard reads the same field off the locally cached session (`getSession()`, no network call) — both with zero extra requests. **`profiles.onboarding_completed_at` stays the single authoritative source** — the `app_metadata` copy is only ever a fast-path mirror, never read anywhere as ground truth. A stale mirror (sync failed, or an old cached session) just means an extra harmless round-trip through the quiz page, which itself treats a resulting `409 already_completed` as success rather than an error.
